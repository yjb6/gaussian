...
class GaussianModel:
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        omega = torch.zeros((fused_point_cloud.shape[0], self.order*3*4), device="cuda") #4个rot，16个多项式basis系数，16个fourierbasis系数
        ...
        motion_poly = torch.zeros((fused_point_cloud.shape[0], 3*self.order), device="cuda")# x1, x2, x3,  y1,y2,y3, z1,z2,z3
    ...
    def cache_gradient(self,stage):
        ...
        if stage == "dynamatic":#只有dynamtic时，才会有下面这三个的梯度
            ...
            self._color_change_grd += self._color_change.grad.clone()
            self.timescale_grd += self.timescale.grad.clone()
    ...
    def training_setup(self, training_args):
        ...
        if training_args.use_weight_decay:
            ...
            {'params': [self._omega], 'lr': training_args.dddm_lr,"weight_decay":8e-7,"name": "omega"},
            ...
            {'params': [self._motion], 'lr':   training_args.dddm_lr , "weight_decay":8e-7,"name": "motion"},
            ...
            l = [
                ...
                {'params': [self._scaling], 'lr': training_args.scaling_lr, "name": "scaling"},
                {'params': [self._rotation], 'lr': training_args.rotation_lr, "name": "rotation"},
                {'params': [self._omega], 'lr': training_args.dddm_lr,"name": "omega"},
                # {'params': [self._trbf_center], 'lr': training_args.trbfc_lr, "name": "trbf_center"},
                # {'params': [self._trbf_scale], 'lr': training_args.trbfs_lr, "name": "trbf_scale"},
                # {'params': [self._motion], 'lr':  training_args.position_lr_init * self.spatial_lr_scale * 0.5 * training_args.movelr , "name": "motion"},
                # {'params': [self._motion_fourier], 'lr':  training_args.position_lr_init * self.spatial_lr_scale * 0.5 * training_args.movelr , "name": "motion_fourier"}
                {'params': [self._motion], 'lr':   training_args.dddm_lr , "name": "motion"},
    ...
    def save_ply(self, path):
        ...
        color_change = self._color_change.detach().cpu().numpy()
        ...
        attributes = np.concatenate((xyz, normals, motion, motion_fourier, f_dc,f_rest, opacities, scale, rotation, omega,color_change,timescale), axis=1)
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_feature_rest ,new_opacities, new_scaling, new_rotation, new_motion, new_motion_fourier,new_omega, new_color_change,new_timescale,dummy=None):
        d = {"xyz": new_xyz,
    ...
    def densify_and_splitv2(self, grads, grad_threshold, scene_extent, N=2):
        ...
        new_color_change = self._color_change[selected_pts_mask].repeat(N,1)
        new_timescale = self.timescale[selected_pts_mask].repeat(N,1)
        ...
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest,new_opacity, new_scaling, new_rotation,  new_motion, new_motion_fourier, new_omega,new_color_change,new_timescale)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        new_color_change = self._color_change[selected_pts_mask]
        new_timescale = self.timescale[selected_pts_mask]
        ...
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest,new_opacities, new_scaling, new_rotation,  new_motion, new_motion_fourier, new_omega,new_color_change,new_timescale)
