...
class GaussianModel:
    ...
    def __init__(self, args,order =16 , rgbfunction="rgbv1"):
        ...
        self.rot_mlp = nn.Sequential(nn.Linear(self.D+out_dim+hexplane_outdim,self.H),nn.ReLU(),nn.Linear(self.H,self.H),nn.ReLU(),nn.Linear(self.H, 4))
    ...
    def save_ply(self, path):
        ...
        torch.save({'motion_state_dict': self.motion_mlp.state_dict(), 'rot_state_dict': self.rot_mlp.state_dict(),
        'hexplane_state_dict':self.hexplane.state_dict()}, model_fname)
...
#            new_trbf_center.append(torch.rand(1).cuda() * torch.ones((Npoints, 1), device="cuda")) 
    ...
    def get_deformation(self,timestamp):
        ...
        # print(self._motion,self._motion.mean(dim=1),self._motion.std(dim=1),self._motion.max(dim=1))
        # print(hexplane_feature.mean(dim=1),hexplane_feature.amax(dim=1))
        # print(self._motion.amax(dim=1))
        ####
        motion_input = torch.cat((hexplane_feature,self._motion,time_embbed),dim=1)
        rot_input = torch.cat((hexplane_feature,self._omega,time_embbed),dim=1)

        # residual = self.normalize_residual(torch.sigmoid(self.motion_mlp(motion_input)))
        residual = self.motion_mlp(motion_input)
        # print(residual)
        motion = self._xyz + residual
        # print(self.get_xyz,motion,residual)
        rot = self._rotation + self.rot_mlp(rot_input)
        # rot = self._rotation.detach() + self.rot_mlp(rot_input)
        return motion, rot
