...
class GaussianModel:
    ...
    def __init__(self, args,order =16 , rgbfunction="rgbv1"):
        ...
        self.base_mlp = nn.Sequential(nn.Linear(self.D + out_dim+hexplane_outdim,self.H),nn.ReLU(),nn.Linear(self.H,1),nn.Sigmoid()) #前两个，一个作为dynamatic,一个作为opcity change
        self.motion_mlp = nn.Sequential(nn.Linear(self.D + out_dim+hexplane_outdim,self.H),nn.ReLU(),nn.Linear(self.H,3))
        # self.motion_mlp = nn.Sequential(nn.Linear(self.D + out_dim,self.H),nn.ReLU(),nn.Linear(self.H, self.H),nn.ReLU(),nn.Linear(self.H, 3))
        self.rot_mlp = nn.Sequential(nn.Linear(self.D + out_dim+hexplane_outdim,self.H),nn.ReLU(),nn.Linear(self.H,7))
        # print(self.H/2)
        self.opacity_mlp = nn.Sequential(nn.Linear(self.D+out_dim+hexplane_outdim,self.H),nn.ReLU(),nn.Linear(int(self.H), 1))#考虑整合进某个别的mlp中
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float,args):
        ...
        for name,W in self.base_mlp.named_parameters():
            if 'bias' in name:
        ...
        start = torch.zeros((fused_point_cloud.shape[0], 1), device="cuda")
        end = torch.ones((fused_point_cloud.shape[0], 1), device="cuda")
        start_end = torch.cat((start,end),dim=1)
        # end = torch.ones((fused_point_cloud.shape[0], 1), device="cuda")
        self.start_end = nn.Parameter(start_end.requires_grad_(True))
        # self.end = nn.Parameter(end.requires_grad_(True))
        # ## store gradients
        ...
        # if self.args.dynamatic_mlp:
        self.dynamatic = torch.zeros_like(self._opacity,device="cuda")
    ...
    def cache_gradient(self,stage):
        ...
        if self.start_end.grad is not None :
            self.start_end_grd += self.start_end.grad.clone()
    ...
    def zero_gradient_cache(self):
        ...
        self.start_end_grd = torch.zeros_like(self.start_end, requires_grad=False)
    ...
    def set_batch_gradient(self, cnt,stage):
        ...
        self.start_end.grad = self.start_end_grd * ratio
        ...
        if stage == "dynamatic":
            ...
            set_mlp_gradient(self.base_mlp,"base")
            set_mlp_gradient(self.hexplane,"hexplane","grids")
            if self.args.dynamatic_mlp:
                set_mlp_gradient(self.dynamatic_mlp,"dynamatic")
            # for name, W in self.motion_mlp.named_parameters():
            #     # if 'weight' in name:
            #         # print(name,W)
    ...
    def training_setup(self, training_args):
        ...
        if training_args.use_weight_decay:
            ...
            {'params': [self.start_end], 'lr': training_args.start_lr ,"name": "start_end"},
            ...
            {'params': list(self.base_mlp.parameters()), 'lr':   training_args.mlp_lr , "weight_decay":8e-7,"name": "base_mlp"},
    ...
    def construct_list_of_attributes(self):
        ...
        for i in range(self.start_end.shape[1]):
            l.append('start_end_{}'.format(i))
    ...
    def save_ply(self, path):
        ...
        start_end = self.start_end.detach().cpu().numpy()
        ...
        attributes = np.concatenate((xyz, normals,  motion,f_dc,f_rest, opacities, scale, rotation, omega,start_end), axis=1)
        ...
        'hexplane_state_dict':self.hexplane.state_dict(),"base_state_dict":self.base_mlp.state_dict()}
        mlp_dict['opacity_state_dict'] = self.opacity_mlp.state_dict()
        if self.args.dynamatic_mlp:
    ...
    def load_ply(self, path):
        ...
        start_end_names = [p.name for p in plydata.elements[0].properties if p.name.startswith("start_end")]
        start_end = np.zeros((xyz.shape[0], len(start_end_names)))
        for idx, attr_name in enumerate(start_end_names):
            start_end[:, idx] = np.asarray(plydata.elements[0][attr_name])
        ...
        self.start_end = nn.Parameter(torch.tensor(start_end, dtype=torch.float, device="cuda").requires_grad_(True))
        ...
        self.base_mlp.load_state_dict({k.replace('base_', ''): v for k, v in mlp_state_dict['base_state_dict'].items()})
        ...
        self.base_mlp.to("cuda")
        self.hexplane.to("cuda")
        # for name ,W in self.hexplane.grids.named_parameters():
        #     print(name,W)
        # exit(0)
        # print("loadok")
        self.opacity_mlp.to("cuda")
    ...
    def _prune_optimizer(self, mask):
        ...
        for group in self.optimizer.param_groups:
            ...
            if len(group["params"]) == 1 and group["name"] != 'decoder' and 'mlp' not in group['name']:
                stored_state = self.optimizer.state.get(group['params'][0], None)
    ...
    def prune_points(self, mask):
        ...
        self._omega = optimizable_tensors["omega"]
        self.start_end = optimizable_tensors["start_end"]
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_feature_rest ,new_opacities, new_scaling, new_rotation, new_motion, new_omega,new_start_end, dummy=None):
        d = {"xyz": new_xyz,
    ...
    def densify_and_splitv2(self, grads, grad_threshold, scene_extent, N=2):
        ...
        new_start_end = self.start_end[selected_pts_mask].repeat(N,1)
        ...
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest,new_opacity, new_scaling, new_rotation,  new_motion, new_omega,new_start_end)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        new_start_end = self.start_end[selected_pts_mask]
        ...
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest,new_opacities, new_scaling, new_rotation,  new_motion, new_omega,new_start_end)
...
#            new_trbf_center.append(torch.rand(1).cuda() * torch.ones((Npoints, 1), device="cuda")) 
    ...
    def get_deformation(self,timestamp):
        ...
        # exit()
        # temp = self.time_emb(torch.tensor([0],dtype=torch.float, device="cuda")).repeat(self._motion.shape[0],1)
        # deform_feature = torch.cat((hexplane_feature,self._motion,temp),dim=1)

        deform_feature = torch.cat((hexplane_feature,self._motion,time_embbed),dim=1)
        base = self.base_mlp(deform_feature)
        # dynamatic = 2*torch.sigmoid(base[:,0].unsqueeze(1))-1
        dynamatic = base[:,0].unsqueeze(1)
        self.dynamatic = dynamatic
        # print(hexplane_feature)
        # print(dynamatic)
        # print(torch.linalg.vector_norm(hexplane_feature,dim=1))
        print(dynamatic.mean().item(),dynamatic.median().item(),dynamatic.amin().item(),dynamatic.amax().item())
        ...
        if self.args.dopacity:
            ...
            # opacity_residual = deform_feature[:,1].unsqueeze(1)
