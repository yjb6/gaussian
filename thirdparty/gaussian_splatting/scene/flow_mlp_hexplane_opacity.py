...
class GaussianModel:
    ...
    def __init__(self, args,order =16 , rgbfunction="rgbv1"):
        ...
        self.motion_mlp = nn.Sequential(nn.Linear(self.D + out_dim+hexplane_outdim,self.H),nn.ReLU(),nn.Linear(self.H,self.H),nn.ReLU(),nn.Linear(self.H, 3))
        # self.motion_mlp = nn.Sequential(nn.Linear(self.D + out_dim,self.H),nn.ReLU(),nn.Linear(self.H, self.H),nn.ReLU(),nn.Linear(self.H, 3))
        self.rot_mlp = nn.Sequential(nn.Linear(self.D+out_dim+hexplane_outdim,self.H),nn.ReLU(),nn.Linear(self.H,self.H),nn.ReLU(),nn.Linear(self.H, 7))
        # print(self.H/2)
        self.opacity_mlp = nn.Sequential(nn.Linear(self.D+out_dim+hexplane_outdim,self.H),nn.ReLU(),nn.Linear(self.H,int(self.H/2) ),nn.ReLU(),nn.Linear(int(self.H/2), 1))#考虑整合进某个别的mlp中
