...
class GaussianModel:
    ...
    def get_rotation(self, t):
        ...
        # omega = self._omega.reshape(self._xyz.shape[0],3,-1) #[,3,n]
        # omega[self.static_mask.squeeze()] = 0
        omega = torch.where(self.static_mask, 0.0, self._omega).reshape(self._xyz.shape[0],3,-1)
        t = torch.tensor([t])
        rot_poly = omega[:,0].reshape(self._xyz.shape[0],4,-1) #[,3,n]
        rot_fourier = omega[:,1:].reshape(self._xyz.shape[0],4,-1,2) #[,3,n,2]
        # motion_poly = self._motion.reshape(self._xyz.shape[0],3,-1) #[,3,n]
        # print(self._xyz.shape)
        # motion_fourier = self._motion_fourier.reshape(self._xyz.shape[0],3,-1,2) #[,3,n,2]
        # temp_list = [t**i for i in range(1,rot_poly.shape[-1]+1)]
        # print(temp_list)
        poly_basis = torch.tensor([t**i for i in range(1,rot_poly.shape[-1]+1)],dtype=torch.float, device="cuda") #[n,]
        fourier_basis = torch.tensor([[torch.sin(i*t),torch.cos(i*t)] for i in range(1,rot_poly.shape[-1]+1)],dtype=torch.float, device="cuda") #[n,2]
        rotation = self._rotation + (rot_poly * poly_basis).sum(-1) + (rot_fourier * fourier_basis).sum([-1,-2])
        # print(motion.shape)
        # motion2 = 
        # print(motion2.shape)
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        omega = torch.zeros((fused_point_cloud.shape[0], 16*3*4), device="cuda") #4个rot，16个多项式basis系数，16个fourierbasis系数
        self._omega = nn.Parameter(omega.requires_grad_(True))
        
        self._opacity = nn.Parameter(opacities.requires_grad_(True))
        
        motion_poly = torch.zeros((fused_point_cloud.shape[0], 3*16), device="cuda")# x1, x2, x3,  y1,y2,y3, z1,z2,z3
        motion_fourier = torch.zeros((fused_point_cloud.shape[0], 3*16*2), device="cuda") #傅里叶系数
        self._motion = nn.Parameter(motion_poly.requires_grad_(True))
        self._motion_fourier = nn.Parameter(motion_fourier.requires_grad_(True))

        static_mask = torch.randint(0, 2, (fused_point_cloud.shape[0],1), dtype=torch.bool,device="cuda")
    ...
    def training_setup(self, training_args):
        ...
        if training_args.use_weight_decay:
            ...
            {'params': [self._omega], 'lr': training_args.omega_lr,"weight_decay":8e-7,"name": "omega"},
            ...
            {'params': [self._motion], 'lr':   training_args.movelr , "weight_decay":8e-7,"name": "motion"},
            {'params': [self._motion_fourier], 'lr': training_args.movelr ,"weight_decay":8e-7,"name": "motion_fourier"}
            ...
            l = [
                ...
                # {'params': [self._motion], 'lr':  training_args.position_lr_init * self.spatial_lr_scale * 0.5 * training_args.movelr , "name": "motion"},
                # {'params': [self._motion_fourier], 'lr':  training_args.position_lr_init * self.spatial_lr_scale * 0.5 * training_args.movelr , "name": "motion_fourier"}
                {'params': [self._motion], 'lr':   training_args.movelr , "name": "motion"},
                {'params': [self._motion_fourier], 'lr': training_args.movelr ,"name": "motion_fourier"}
    ...
    def save_ply(self, path):
        ...
        attributes = np.concatenate((xyz, normals, motion, motion_fourier, f_dc, opacities, scale, rotation, omega,static_mask), axis=1)
    ...
    def load_ply(self, path):
        ...
        xyz = np.stack((np.asarray(plydata.elements[0]["x"]),
                        ...
                        np.asarray(plydata.elements[0]["z"])),  axis=1)
        opacities = np.asarray(plydata.elements[0]["opacity"])[..., np.newaxis]
        static_mask = np.asarray(plydata.elements[0]["static_mask"])[..., np.newaxis]
            #         {'params': [self._trbf_center], 'lr': training_args.trbfc_lr, "name": "trbf_center"},
            # {'params': [self._trbf_scale], 'lr': training_args.trbfs_lr, "name": "trbf_scale"},
        # trbf_center= np.asarray(plydata.elements[0]["trbf_center"])[..., np.newaxis]
        # trbf_scale = np.asarray(plydata.elements[0]["trbf_scale"])[..., np.newaxis]
        ...
        self.static_mask = torch.tensor(static_mask, dtype=torch.bool, device="cuda")
    ...
    def prune_points(self, mask):
        ...
        self.static_mask = self.static_mask[valid_points_mask]
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_opacities, new_scaling, new_rotation, new_motion, new_motion_fourier,new_omega,new_static_mask, dummy=None):
        d = {"xyz": new_xyz,
        ...
        "motion": new_motion,
        "motion_fourier": new_motion_fourier,
        "omega": new_omega}
        ...
        self.static_mask = torch.cat((self.static_mask, new_static_mask), dim=0)
    ...
    def densify_and_splitv2(self, grads, grad_threshold, scene_extent, N=2):
        ...
        new_motion_fourier = self._motion_fourier[selected_pts_mask].repeat(N,1)

        new_omega = self._omega[selected_pts_mask].repeat(N,1)
        new_static_mask = self.static_mask[selected_pts_mask].repeat(N,1)
        ...
        self.densification_postfix(new_xyz, new_features_dc, new_opacity, new_scaling, new_rotation,  new_motion, new_motion_fourier, new_omega,new_static_mask) #新的点加入

        prune_filter = torch.cat((selected_pts_mask, torch.zeros(N * selected_pts_mask.sum(), device="cuda", dtype=bool))) #原来的点过滤
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        new_motion_fourier = self._motion_fourier[selected_pts_mask]

        new_omega = self._omega[selected_pts_mask]

        new_static_mask = self.static_mask[selected_pts_mask]
        ...
        self.densification_postfix(new_xyz, new_features_dc, new_opacities, new_scaling, new_rotation,  new_motion, new_motion_fourier, new_omega,new_static_mask)
...
#            new_trbf_center.append(torch.rand(1).cuda() * torch.ones((Npoints, 1), device="cuda")) 
    ...
    def get_motion(self, t):
        ...
        # motion_poly = self._motion.reshape(self._xyz.shape[0],3,-1) #[,3,n]
        motion_poly= torch.where(self.static_mask, 0.0, self._motion).reshape(self._xyz.shape[0],3,-1)

        # motion_poly[self.static_mask.squeeze()] =0
        # print(self._xyz.shape)
        # motion_fourier = self._motion_fourier.reshape(self._xyz.shape[0],3,-1,2) #[,3,n,2]
        motion_fourier= torch.where(self.static_mask, 0.0, self._motion_fourier).reshape(self._xyz.shape[0],3,-1,2)

        # motion_fourier[self.static_mask.squeeze()] =0
        temp_list = [t**i for i in range(1,motion_poly.shape[-1]+1)]
        # print(temp_list)
        poly_basis = torch.tensor(temp_list,dtype=torch.float, device="cuda") #[n,]
        fourier_basis = torch.tensor([[torch.sin(i*t),torch.cos(i*t)] for i in range(1,motion_poly.shape[-1]+1)],dtype=torch.float, device="cuda") #[n,2]
        motion = self.get_xyz + (motion_poly * poly_basis).sum(-1) + (motion_fourier * fourier_basis).sum([-1,-2])
        # print(motion.shape)
        # motion2 = 
        # print(motion2.shape)
        return motion
    ...
    def adptive_timescale(self,t):
        return self.timescale[0] *t + self.timescale[1]
    
    def get_time_smoothloss(self,t):
        current_loc = self.get_motion(t)
        current_rot = self.get_rotation(t)
        delta_t = t + 0.1/self.duration
        next_loc = self.get_motion(delta_t)
        next_rot = self.get_rotation(delta_t)
        # print(next_loc.shape,next_rot.shape)
        smoothloss = (torch.norm(current_loc - next_loc, p=2,dim=1) + torch.norm(current_rot - next_rot, p=2,dim=1)).mean()
        return smoothloss
