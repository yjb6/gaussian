...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int,order =16 , rgbfunction="rgbv1"):
        ...
        self.D=32
        self.H=128
        self.time_emb ,time_out_dim = get_embedder(4,1)
        self.xyz_emb,xyz_out_dim = get_embedder(8,3)
        # self.rot_emb,rot_out_dim = get_embedder(3,4)
        # self.motion_fc1 = nn.Linear(self.D+time_out_dim, xyz_out_dim)
        # self.motion_fc2 = nn.Sequential(nn.Linear(xyz_out_dim, self.H),nn.ReLU(),nn.Linear(self.H, 3))
    ...
    def training_setup(self, training_args):
        ...
        if training_args.use_weight_decay:
            ...
            # {'params': list(self.motion_fc1.parameters()), 'lr':   training_args.mlp_lr_init , "weight_decay":8e-7,"name": "motion_mlp"},
            # {'params': list(self.motion_fc2.parameters()), 'lr':   training_args.mlp_lr_init , "weight_decay":8e-7,"name": "motion_mlp"},
            ...
            l = [
                ...
                # {'params': list(self.motion_fc1.parameters()), 'lr':   training_args.mlp_lr , "name": "motion_mlp"},
                # {'params': list(self.motion_fc2.parameters()), 'lr':   training_args.mlp_lr , "name": "motion_mlp"},
...
#            new_trbf_center.append(torch.rand(1).cuda() * torch.ones((Npoints, 1), device="cuda")) 
    ...
    def get_motion(self, timestamp):
        ...
        motion_input = torch.cat((xyz_embbed,self._motion,time_embbed),dim=1)
        rot_input = torch.cat((self._omega,time_embbed),dim=1)
        motion_out = self.inv_normalize_xyz(torch.sigmoid(self.motion_mlp(motion_input)))
            # motion = motion_out
            # rot = self.rotation_activation(self._rotation+self.rot_mlp(rot_input))
        return motion_out
    ...
    def get_deformation(self,timestamp,stage):
        ...
        if stage == "static":
            ...
            motion_input = torch.cat((xyz_embbed.detach(),zero_feature),dim=1)
            # print(motion_input.shape)
            motion_out = self.motion_mlp(motion_input)
            # print(motion_out)
            ...
            # exit()
            zero_feature = torch.zeros((self._xyz.shape[0],(time_embbed.shape[1]+self.D)),device="cuda")
            motion_input = torch.cat((xyz_embbed,zero_feature),dim=1)
