...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, near, far, startime=0, duration=50):
    ...
    for idx, key in enumerate(cam_extrinsics): # first is cam20_ so we strictly sort by camera name
        ...
        for j in range(startime, startime+ int(duration)):
            ...
            #这里的改动是让所有的cam_info都去记录相机位姿，因为后面再去遍历dataset代价太大，太慢
            ...
            # if j == startime:
            #     #这里只让starttime记录相机位姿，因为后面的都是一样的
...
def readColmapSceneInfoImmersive(path, images, eval, llffhold=8, multiview=False, duration=50, testonly=False ):
    ...
    if not os.path.exists(totalply_path):
        ...
        storePly(totalply_path, xyzt, rgb)#创建结构化点云
    try:
...
def readColmapSceneInfoHyperNerf(path, images, eval, llffhold=8, multiview=False, duration=50,time='zeros'):
    use_bg_points = False
    train_cam_infos = Load_hyper_data(path,0.5,use_bg_points,split ="train")
    test_cam_infos = Load_hyper_data(path,0.5,use_bg_points,split="test")
    print("load finished")
    train_cam = format_hyper_data(train_cam_infos,"train")
    test_cam = format_hyper_data(train_cam_infos,"test")
    ...
    if not os.path.exists(timeply_path):
        #加入时间信息
        if time == 'zeros':
            totaltime = np.zeros((xyz.shape[0],1))
        assert xyz.shape[0] == rgb.shape[0]  
        # print(xyz.shape[0])
        xyzt =np.concatenate( (xyz, totaltime), axis=1)     
        storePly(timeply_path, xyzt, rgb)
    try:
        pcd = fetchPly(timeply_path)
    except:
        pcd = None
    
    nerf_normalization = getNerfppNorm(train_cam)
    plot_camera_orientations(train_cam, pcd.points)
    scene_info = SceneInfo(point_cloud=pcd,
                           train_cameras=train_cam,
                           test_cameras=test_cam,
                        #    video_cameras=video_cam_infos,
                           nerf_normalization=nerf_normalization,
                           ply_path=timeply_path,
                        #    maxtime=max_time
                           )
...
def readColmapSceneInfo4dgs(path, images, eval, llffhold=8, multiview=False, duration=50):
    '''不再将每一帧的点云带着时间去融合，而是用4dgs的方法，用第一帧只建一个点云'''
    try:
        cameras_extrinsic_file = os.path.join(path, "sparse/0", "images.bin")
        cameras_intrinsic_file = os.path.join(path, "sparse/0", "cameras.bin")
        cam_extrinsics = read_extrinsics_binary(cameras_extrinsic_file) 
    ...
    ply_path = os.path.join(os.path.dirname(path),"points3D_downsample2.ply")
    pcd = fetchPly3D(ply_path)
    xyz = np.array(pcd.points)
    rgb = np.array(pcd.colors)
    timeply_path = os.path.join(os.path.dirname(path), "points3D"  + ".ply")
    # pcd = pcd._replace(points=xyz)
    if not os.path.exists(timeply_path):
        #加入时间信息
        # if time == 'zeros':
...
sceneLoadTypeCallbacks = {
    ...
    "hypernerf": readColmapSceneInfoHyperNerf,
    "Colmap4dgs": readColmapSceneInfo4dgs
}
