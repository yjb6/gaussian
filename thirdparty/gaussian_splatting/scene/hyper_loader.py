...
from utils.hypernerf_utils import Camera
...
def format_hyper_data(data_class, split):
    ...
    for uid, index in tqdm(enumerate(data_idx)):
        ...
        cam_info = CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, image=image,
                              ...
                              height=int(data_class.h), timestamp=time, mask=mask, near=data_class.near, far=data_class.far,
                              pose=1, hpdirecitons=1,cxr=0.0, cyr=0.0
                              )
        # print(cam_info.height, cam_info.width)
        # else:
        #     cam_info = CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, image=image,
        #                       image_path=image_path, image_name=image_name, width=int(data_class.w), 
        #                       height=int(data_class.h), timestamp=time, mask=mask, near=data_class.near, far=data_class.far,
