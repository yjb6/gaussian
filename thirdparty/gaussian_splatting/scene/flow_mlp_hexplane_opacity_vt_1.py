...
from scene.hexplane_vt import HexPlaneField_vt
class GaussianModel:
    ...
    def __init__(self, args,order =16 , rgbfunction="rgbv1"):
        ...
        elif self.args.planemodel =="hexplane_vt":
            self.hexplane = HexPlaneField_vt(args.bounds, args.kplanes_config, args.multires)
        else:
        ...
        # print(self.H/2)
        self.opacity_mlp = nn.Sequential(nn.Linear(out_dim+hexplane_outdim,self.H),nn.ReLU(),nn.Linear(self.H,int(self.H/2) ),nn.ReLU(),nn.Linear(int(self.H/2), 1))#考虑整合进某个别的mlp中
    ...
    def get_trbfscale(self):
        return torch.sigmoid(self._trbf_scale)
    @property
    ...
    def set_batch_gradient(self, cnt,stage):
        ...
        # print("t_center_grad",self._trbf_center.grad.mean(),self._trbf_center.grad.std(),self._trbf_center.grad.max(),self._trbf_center.grad.min())
        # print("t_center_grad_nan",torch.isnan(self._trbf_center.grad).sum())
    ...
    def update_learning_rate(self, iteration):
        ...
        for param_group in self.optimizer.param_groups:
            ...
            #         max_scale = self._trbf_scale.max()
            #         per_point_lr = lr* self._trbf_scale /max_scale
            #         param_group['lr'] = lr
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_feature_rest ,new_opacities, new_scaling, new_rotation, new_trbf_center, new_trbfscale,new_motion, new_omega, dummy=None):
        d = {"xyz": new_xyz,
...
#            new_trbf_center.append(torch.rand(1).cuda() * torch.ones((Npoints, 1), device="cuda")) 
    ...
    def get_deformation(self,timestamp):
        #利用s=vt来算出位移。首先确定出时间间隔，为一帧。然后算timestamp与t_center中间隔了几帧
        ####
        # time = torch.full((self._motion.shape[0],1),timestamp).to("cuda")
        # hexplane_feature = self.hexplane(self._xyz,self._trbf_center ) #[N,D]
        trbf_center_clamp = torch.clamp(self._trbf_center,0,1)
        trbfdistanceoffset = timestamp  - trbf_center_clamp #时间间隔[N,1]
        # print("t_center",self._trbf_center)
        # print("t_mean",self._trbf_center.mean(),self._trbf_center.max(),self._trbf_center.min())
        # t_isnan = self._trbf_center.isnan().sum()
        # (index_11,index_22) = torch.nonzero(self._trbf_center.isnan(),as_tuple=True)
        # print(index_11,index_22)
        # print('t_nan',t_isnan)
        # assert t_isnan == 0
        # sign_tensor = torch.sign(trbfdistanceoffset)
        # print(sign_tensor.shape)
        dt = 5/self.duration
        t_point_num = (abs(trbfdistanceoffset) / dt).ceil().to(torch.int32)  #每个点的采样时间数
        t_point_num[t_point_num==0] = 1 #最小值设为1
        point_dt = trbfdistanceoffset/t_point_num #每个点的时间间隔
        # point_dt[point_dt.isnan()] = 0 #有可能会有除0的风险，将nan值设为0
        # print(point_dt.shape)
        start_t = trbf_center_clamp + point_dt/2

        point_dt_matrix = point_dt * torch.arange(t_point_num.max().cpu().item(),dtype=torch.float,device="cuda") #[N,M]
        # print(point_dt_matrix.shape)
        time_select = start_t + point_dt_matrix
        # time_select[time_select>(timestamp*sign_tensor)] = -1
        # time_select_list = []
        # (index_1_,index_2_) = torch.nonzero(torch.logical_and(time_select<(timestamp),time_select>(timestamp)),as_tuple=True)
        index_matrix = torch.arange(t_point_num.max().cpu().item(),device="cuda").unsqueeze(0).repeat(t_point_num.shape[0],1)
        (index_1,index_2) = torch.nonzero(index_matrix < t_point_num,as_tuple=True)
        # print(time_select[:2])
        # print(t_point_num[:2])
        # print(index_1_.shape)
        # print(index_1_[:100])  
        # index_1 = torch.arange(self._xyz.shape[0],dtype=torch.int32).unsqueeze(1).to('cuda')

        # print(index_1.device, t_point_num.device)
        # index_1 = torch.repeat_interleave(index_1, t_point_num.squeeze(1), dim=0).squeeze()
        # print(index_1.shape)
        # print(index_1[:100])
        # print("start")
        # index_2 = torch.cat([torch.arange(val.item()) for val in t_point_num]).squeeze()
        # assert index_1_ == index_1,print(index_1_,index_1)
        # assert index_2_ == index_2,print(index_2_,index_2)

        # print("end")
        t_valid_values = time_select[index_1,index_2]
        # print(t_point_num.shape)
        # t_point_num_repeat = torch.repeat_interleave(t_point_num,3,dim=0)
        # print(t_point_num_repeat.shape)
        xyz_repeat = torch.repeat_interleave(self._xyz, t_point_num.squeeze(1), dim=0)
        point_dt_repeat = torch.repeat_interleave(abs(point_dt), t_point_num.squeeze(1), dim=0)
        # xyz_repeat = xyz_repeat.reshape(-1,3)
        # print(xyz_repeat.shape)
        space_feature,time_feature = self.hexplane(self._xyz,xyz_repeat,t_valid_values.unsqueeze(1)) #[N,D]
        # print(time_feature.shape)
        time_feature_matrix = torch.zeros((time_select.shape[0],time_select.shape[1],time_feature.shape[1]),device="cuda")
        time_feature_matrix[index_1,index_2] = time_feature
        # print(time_feature_matrix.shape)
        # print(time_feature_matrix.sum(dim=1).shape)
        # print(t_point_num.shape)
        # print("t_num_min",t_point_num.min())
        time_feature_mean = time_feature_matrix.sum(dim=1)/t_point_num
        # time_feature_mean[time_feature_mean.isnan()] = 0
        time_embbed = self.time_emb(abs(trbfdistanceoffset))
        # print(space_feature.shape,time_feature_mean.shape,time_embbed.shape)
        # s_nan = space_feature.isnan().sum()
        # t_nan = time_feature_mean.isnan().sum()
        # print("s_f_nan",s_nan)
        # print("t_f_nan",t_nan)
        # assert s_nan == 0
        # assert t_nan == 0
        deform_feature = torch.cat((space_feature,time_feature_mean,time_embbed),dim=1)
        # point_dt_valid = point_dt_matrix[index_1,index_2].unsqueeze(1)
        # print(point_dt_valid.shape)
        # time_embbed = self.time_emb(point_dt_valid)
        # print(time_embbed.shape)
        # deform_feature = torch.cat((hexplane_feature,time_embbed),dim=1)

        # trbfdistance =  trbfdistanceoffset / self.get_trbfscale
        # trbfoutput = torch.exp(-1*(trbfdistance**2))
        ...
        # # time_embbed = torch.zeros_like(time_embbed,device="cuda")
        # # time_embbed = time_embbed.repeat(self._motion.shape[0],1)
        # deform_feature = torch.cat((hexplane_feature,self._motion,time_embbed.detach()),dim=1)
        # num_feature = deform_feature.shape[0]
        # batch_size =100000
        # pad_num = 32 - num_feature % 32
        # if pad_num != 32:
        #     print(pad_num)
        #     padding_zeros = torch.zeros((pad_num,deform_feature.shape[-1]),device="cuda")
        #     print(padding_zeros.shape)
        #     deform_feature = torch.cat((deform_feature,padding_zeros),dim=0)

        # deform_feature = deform_feature.reshape(batch_size,-1,deform_feature.shape[-1])
        # print(deform_feature.shape)

        # batch_len = deform_feature.shape[0]//batch_size
        # motion_residual_list=[]
        # rot_residual_list=[]
        # opacity_residual_list=[]
        # for i in range(batch_len):
        #     if i == batch_len-1:
        #         batch_deform_feature = deform_feature[i*batch_size:]
        #     else:
        #         batch_deform_feature = deform_feature[i*batch_size:(i+1)*batch_size]
        #     motion_residual_list.append(self.motion_mlp(batch_deform_feature).to("cpu"))
        #     rot_residual_list.append(self.rot_mlp(batch_deform_feature).to("cpu"))
        #     opacity_residual_list.append(self.opacity_mlp(batch_deform_feature).to("cpu"))
        # deform_feature = deform_feature.to("cpu")
        # motion_residual = torch.cat(motion_residual_list,dim=0).to("cpu")
        # rot_residual = torch.cat(rot_residual_list,dim=0).to("cpu")
        # opacity_residual = torch.cat(opacity_residual_list,dim=0).to("cpu")
        # motion_residual_list.to("cpu")
        # rot_residual_list.to("cpu")
        # opacity_residual_list.to("cpu")
        if self.args.dx:
            # motion_residual_list.to("cuda")
            # print(deform_feature.shape)
            ...
            #     else:
            #         batch_deform_feature = deform_feature[i*batch_size:(i+1)*batch_size]
            #     motion_residual_list.append(self.motion_mlp(batch_deform_feature))
            # motion_residual = torch.cat(motion_residual_list,dim=0)
            # print("deform_feat",deform_feature)
            motion_residual = self.motion_mlp(deform_feature)
            # print("motion",motion_residual)
            # motion_residual = motion_residual.to("cuda")
            # print("dt",abs(trbfdistanceoffset))
            motion_residual = motion_residual*abs(trbfdistanceoffset)
            # print(motion_residual.shape)
            # motion_residual = self.motion_mlp(deform_feature).reshape(-1,3)
            # if pad_num !=32:
            #     motion_residual=motion_residual[:-pad_num]
            # motion_matrix = torch.zeros((time_select.shape[0],time_select.shape[1],3),device="cuda")
            # motion_matrix[index_1,index_2] = motion_residual
            # print(motion_matrix.shape)
            # print(point_dt.shape)
            # motion_matrix = motion_matrix*abs(point_dt).unsqueeze(2)
            # motion_residual = motion_matrix.sum(dim=1)
        ...
        if self.args.drot:
            ...
            rot_residual = rot_residual*abs(trbfdistanceoffset)
            # print(rot_residual)
            # rot_residual = rot_residual.to("cuda")
            # rot_residual = rot_residual*point_dt_repeat
            # rot_residual = self.rot_mlp(deform_feature).reshape(-1,7)
            # rot_residual = torch.cat(rot_residual_list,dim=0)
            # if pad_num !=32:
            #     rot_residual=rot_residual[:-pad_num]
            # rot_matrix = torch.zeros((time_select.shape[0],time_select.shape[1],7),device="cuda")
            # rot_matrix[index_1,index_2] = rot_residual
            # # rot_matrix = rot_matrix*abs(point_dt).unsqueeze(2)
            # rot_residual = rot_matrix.sum(dim=1)
        ...
        if self.args.dopacity:
            ...
            opacity_residual = self.opacity_mlp(deform_feature)
            # opacity_residual = opacity_residual.to("cuda")
            opacity_residual = opacity_residual*abs(trbfdistanceoffset)

            # print(opacity_residual)
            # print(self.opacity_mlp.state_dict())
            #按原来的想法，dopacity是在原来的opacity上乘了一个k，那么不同时刻下的k就要累乘。表示当前时刻相对于上一个时刻opacity的变化率。
            #当然也可以实现累加,表示opaciy随时间的变化。累加应作用于加sigmoid之前。累乘应作用于之后
            # opacity_residual = self.opacity_mlp(deform_feature).reshape(-1,1) #这个的bias要初始化为1
            # opacity_residual = torch.cat(opacity_residual_list,dim=0)
            # if pad_num !=32:
            #     opacity_residual=opacity_residual[:-pad_num]
            # opacity_residual = torch.sigmoid(opacity_residual)
            # opacity_matrix = torch.ones((time_select.shape[0],time_select.shape[1],1),device="cuda")
            # opacity_matrix[index_1,index_2] = opacity_residual
            # opacity_matrix = opacity_matrix*abs(point_dt).unsqueeze(2)
            # opacity_residual = opacity_matrix.prod(dim=1)
            # opacity_residual = trbfoutput
            # print("opacity_residual",opacity_residual)
            opacity = self._opacity + opacity_residual
            # print(self._opacity)
            # print(opacity_residual)
            # * opacity_residual
            # *(1-trbfoutput)
            # + opacity_residual
            # print(opacity_residual)
            # print(opacity_residual[select_mask.squeeze()])
            # print("o",opacity)
            # print(trbfoutput.mean(),trbfoutput.max(),trbfoutput.min())
            # valid_mask = (trbfoutput>0.01).squeeze()
            # valid_mask = (abs(trbfdistanceoffset) < 15/300).squeeze()
            # valid_tensor = torch.zeros_like(opacity,requires_grad=False)
            # valid_tensor[valid_mask] = 1
            # print(timestamp,valid_tensor.sum())
            # print(opacity_residual)
