...
class GaussianModel:
    ...
    def update_learning_rate(self, iteration,stage=None):
        ...
        #     # print(self.inv_intergral,self.inv_intergral.max(),self.inv_intergral.min())
            ...
            # # print(self.inv_intergral[torch.logical_and(self._trbf_center>=0,self._trbf_center<=1)].max())
            # print(self._trbf_center.min())
            # print(self._trbf_scale.min())
            # print(self._trbf_center[self.inv_intergral==torch.inf])
...
#            new_trbf_center.append(torch.rand(1).cuda() * torch.ones((Npoints, 1), device="cuda")) 
    ...
    def get_deformation(self,timestamp):
        ...
        trbf_scale = (1-min_scale)*trbf_scale + min_scale #限制min_scale最小值
        self._trbf_scale = trbf_scale
        # print(self._trbf_scale.mean(),self._trbf_scale.min(),self._trbf_scale.max())
        # print(trbf_scale,trbf_scale.min(),trbf_scale.max(),trbf_scale.mean())
        # print(self.get_trbfscale.mean(),self.get_trbfscale.max(),self.get_trbfscale.min())
        # print(self._trbf_center.mean(),self._trbf_center.max(),self._trbf_center.min())
