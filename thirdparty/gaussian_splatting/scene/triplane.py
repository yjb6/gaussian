...
def grid_sample_wrapper(grid: torch.Tensor, coords: torch.Tensor, align_corners: bool = True) -> torch.Tensor:
    ...
    n = coords.shape[-2]
    interp = grid_sampler(
        grid,  # [B, feature_dim, reso, ...] #[N,C,H,W]
        coords,  # [B, 1, ..., n, grid_dim] #这个对应的是（x,y）坐标，x对应的是W，y对应的是H。所以y这个对应的是时间维度的
...
def init_grid_param(
    ...
    coo_combs = [(i,in_dim-1) for i in range(in_dim-1)] #最后一个维度为时间维度，构建 x-t,y-t,z-t平面
